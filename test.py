...
def read_grad(model, path):
    diff_file = f"{model}/{path}/diff.txt"
    ...
    grad_cpu = np.load(f'{model}/{path}/case/paddle_cpu/grad.npz')
    grad_gpu = np.load(f'{model}/{path}/case/paddle_gpu/grad.npz')

    output_cpu = np.load(f'{model}/{path}/case/paddle_cpu/output.npz')
    output_cpu = output_cpu['output']
    output_gpu = np.load(f'{model}/{path}/case/paddle_gpu/output.npz')
    output_gpu = output_gpu['output']

    output_diff = chebyshev_distance(output_cpu, output_gpu)
        ...
        for key in keys_cpu:
            ...
            if compare_result == 100:
                ...
                if grad_cpu[key] is None:
                    print(f"grad_cpu[{key}] is none")
                    write_file(diff_file, f"grad_cpu[{key}] is none\n")
                elif grad_gpu[key] is None:
                    print(f"grad_gpu[{key}] is none")
                    write_file(diff_file, f"grad_gpu[{key}] is none\n")
                else:
                    # print(f"{key}: 梯度数据一致")
                    no_use = 1
            else:
